...
public class FrequencyPerArticleStrategy implements WordCountStrategy {
    ...
    public List<Map.Entry<String, Integer>> execute(List<Article> articles) { //Ritorna una lista costituita da coppia stringa valore
        ...
        for (Article a : articles) {
            ...
            List<String> body = Arrays.asList(a.getBody().toLowerCase().replaceAll("[^a-z0-9\\s]", "").split(" ")); //passo da array a lista, dove mi serve per invocare in seguito la creazione del set, chiamo a cascata vari metodi per flitrare e buttare in un array con split
